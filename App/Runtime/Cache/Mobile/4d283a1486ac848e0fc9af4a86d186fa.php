    ...
    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="wyqd">
            <div class="wyqd_item">
                ...
                <div class="jlsh">
                    ...
                    <?php if($vo['task_id'] > 0): if($vo['status'] == 1): ?><a href="javascript:;" onclick="re_buy(<?php echo ($vo['id']); ?>,2)"><span>复购</span></a><em></em>
                          <?php else: ?>
                            <a href="<?php echo U('Trade/pay_sk',array('task_id'=>$vo['task_id'],'status'=>$vo['status']));?>"><span>去收款</span></a><em></em><?php endif; ?>
                    <?php else: ?> <!--系统匹配-->
                        <?php if($vo['status'] == 1): ?><a href="javascript:;"><span>收取成功</span></a><em></em>
                         <?php else: ?>
                            <a href="javascript:;"><span>等待系统支付</span></a><em></em><?php endif; endif; ?>
                </div>
            </div>
