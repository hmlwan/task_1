...
class SafeController extends HomeController {
	...
	public function index(){
		...
		$authentication = M('member_photo')->where(array('member_id'=>$_SESSION['USER_KEY_ID']))->order('id desc')->find();
		//$is_auth = !empty($authentication) ? 1 : 0;
		$this->assign('is_auth', $authentication);
        $this->display();
     }
     /**
      * 获取当前货币交易记录的最新信息
