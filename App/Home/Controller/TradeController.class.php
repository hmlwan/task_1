...
class TradeController extends TradeFatherController {
	...
	public function index(){
		...
		$currency=$this->currency->where("currency_mark='{$_GET['currency']}' and is_lock=0")->find();
		if(empty($currency)){
			$this->display('Public:b_stop');
            return;
		}
		...
		$sell_record=$this->getOrdersByType($currency['currency_id'],'sell', 10, 'desc');
		...
		$comment=new MemberCommentController();
		$membercomment=$comment->getMemberCommentByCurrencyid($currency['currency_id']);
		...
		//成交记录
		$trade=$this->getTradesByNum(20,$currency['currency_id']);
...
//         }
      ...
      //  M()->query('lock tables yang_orders write, yang_currency_user write');
        ...
        //挂单流程
       $r[]=$this->guadan($buynum, $buyprice, 'buy', $currency,$this->member['member_id']);
 ...
 //      M()->query("UNLOCK TABLES");
       if (in_array(false, $r)){
           ...
           $msg['status']=1;
           $msg['info']='操作成功';
           $_SESSION['transaction_again']=$buycurrency_id;//是否交易过
           $this->ajaxReturn($msg);
       }
	...
	public function sell(){
	   ...
	   if ($currency['is_lock']){
        ...
        //写入数据库
		$r[]=$this->guadan($sellnum, $sellprice, 'sell', $currency,$this->member['member_id']);
		//成交
		...
		if (in_array(false, $r)){
		    ...
		    $msg['status']=1;
		    $msg['info']='操作成功';
		    $_SESSION['transaction_again']=$currency_id;//是否交易过
		    $this->ajaxReturn($msg);
		}
	} 
     ...
     * @param $currency_id 交易币种
     */
    private function guadan($num,$price,$type,$currency,$member_id=0){
        //获取交易币种信息
        switch ($type){
            case 'buy':
                //等级买入交易手续费
                $grade_id=M('member')->where(['member_id'=>$member_id])->getField('grade_id');
                $trade_fee_buy=M('member_grade')->where(['id'=>$grade_id])->getField('trade_fee_buy')?:0;
                $fee=$trade_fee_buy/100;
            ...
            case 'sell':
                //等级卖出交易手续费
                $grade_id=M('member')->where(['member_id'=>$member_id])->getField('grade_id');
                $trade_fee_sell=M('member_grade')->where(['id'=>$grade_id])->getField('trade_fee_sell')?:0;
                $fee=$trade_fee_sell/100;
    ...
    private function trade($currencyId,$type,$num,$price){
       ...
       //如果没有相匹配的订单，直接返回
        ...
        switch ($type){
            ...
            case 'buy':
               ...
               $r[]=$this->setUserMoney($this->member['member_id'],$order['currency_trade_id'], $trade_num*($order['price']-$trade_price), 'inc', 'num');
                ...
                //下级买入奖励上级手续费
                //获取操作人一个订单--->买 -->给币
                $uuser=M('member')->where(['member_id'=>$order['member_id']])->find();
                $up_user=M('member')->where(['member_id'=>$uuser['pid']])->find();
                if ($up_user) {
                    $shouxufei = $trade_num*$order['fee'];
                    if ($shouxufei>0) {
                        $shezhi=M('member_grade')->where(['id'=>$up_user['grade_id']])->getField('up_trade_fee_buy')?:0;
                        $shouxufei_up=round($shouxufei*$shezhi/100,3);
                        if ($shouxufei_up>0) {
                            //加币
                            $this->setUserMoney($up_user['member_id'],$order['currency_id'], $shouxufei_up, 'inc', 'num');
                            //记录
                            $r[]=$this->addFinance($up_user['member_id'], 101, '下级UID:'.$uuser['member_id'].'买入奖励交易手续费',$shouxufei_up, 1, $order['currency_id']);
                        }
                    }
                }
                //获取对应交易的一个订单--->卖 ->给对应钱
                $uuser=M('member')->where(['member_id'=>$trade_order['member_id']])->find();
                $up_user=M('member')->where(['member_id'=>$uuser['pid']])->find();
                if ($up_user) {
                    $shouxufei = $trade_num*$trade_price*$trade_order['fee'];
                    if ($shouxufei>0) {
                        $shezhi=M('member_grade')->where(['id'=>$up_user['grade_id']])->getField('up_trade_fee_sell')?:0;
                        $shouxufei_up=round($shouxufei*$shezhi/100,3);
                        if ($shouxufei_up>0) {
                            //jiaqian
                            $this->setUserMoney($up_user['member_id'],$trade_order['currency_trade_id'], $shouxufei_up, 'inc', 'num');
                            //log
                            $r[]=$this->addFinance($up_user['member_id'], 101, '下级UID:'.$uuser['member_id'].'卖出奖励交易手续费',$shouxufei_up, 1, $order['currency_id']);
                        }
                    }
                }
            ...
            case 'sell':
                ...
                $r[]=$this->setUserMoney($trade_order['member_id'],$trade_order['currency_id'], $trade_num*(1-$order['fee']), 'inc', 'num');
                $r[]=$this->setUserMoney($trade_order['member_id'],$trade_order['currency_trade_id'],$trade_order_money, 'dec', 'forzen_num');
                //手续费
                $r[]=$this->addFinance($order['member_id'], 11, '交易手续费',$trade_num*$order['price']*$order['fee'], 2, $order['currency_trade_id']);
                $r[]=$this->addFinance($trade_order['member_id'], 11, '交易手续费',$trade_num*$trade_order['fee'], 2, $order['currency_id']);

                //下级卖出奖励上级手续费
                //获取操作人一个订单--->卖 -->gei qian
                $uuser=M('member')->where(['member_id'=>$order['member_id']])->find();
                $up_user=M('member')->where(['member_id'=>$uuser['pid']])->find();
                if ($up_user) {
                    $shouxufei = $trade_num*$order['price']*$order['fee'];
                    if ($shouxufei>0) {
                        $shezhi=M('member_grade')->where(['id'=>$up_user['grade_id']])->getField('up_trade_fee_sell')?:0;
                        $shouxufei_up=round($shouxufei*$shezhi/100,3);
                        if ($shouxufei_up>0) {
                            $this->setUserMoney($up_user['member_id'],$order['currency_trade_id'], $shouxufei_up, 'inc', 'num');
                            $r[]=$this->addFinance($up_user['member_id'], 101, '下级UID:'.$uuser['member_id'].'卖出奖励交易手续费',$shouxufei_up, 1, $order['currency_id']);
                        }
                    }
                }
                //获取对应交易的一个订单--->买 ->gei bi
                $uuser=M('member')->where(['member_id'=>$trade_order['member_id']])->find();
                $up_user=M('member')->where(['member_id'=>$uuser['pid']])->find();
                if ($up_user) {
                    $shouxufei = $trade_num*$trade_order['fee'];
                    if ($shouxufei>0) {
                        $shezhi=M('member_grade')->where(['id'=>$up_user['grade_id']])->getField('up_trade_fee_buy')?:0;
                        $shouxufei_up=round($shouxufei*$shezhi/100,3);
                        if ($shouxufei_up>0) {
                            $this->setUserMoney($up_user['member_id'],$trade_order['currency_id'], $shouxufei_up, 'inc', 'num');
                            $r[]=$this->addFinance($up_user['member_id'], 101, '下级UID:'.$uuser['member_id'].'买入奖励交易手续费',$shouxufei_up, 1, $order['currency_id']);
                        }
                    }
                }
        ...
        $r[]=$this->addTrade($trade_order['member_id'], $trade_order['currency_id'], $trade_order['currency_trade_id'], $trade_price, $trade_num, $trade_order['type'],$order['fee']);       
        $num =$num- $trade_num;
        if ($num>0){
            //递归
    ...
    private function  addTrade($member_id,$currency_id,$currency_trade_id,$price,$num,$type,$fee){
        $fee=$price*$num*$fee;
     //   $this->dividend($price*$num,$member_id);  //时代比中的分红程序
        $data=array(
            'member_id'=>$member_id,
