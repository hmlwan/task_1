...
class OrdersController extends CommonController{
    ...
    public function index(){
        ...
        //显示委托记录
        $buy_record=$this->getOrdersByType($currency['currency_id'],'buy', 15, 'desc');
        $sell_record=$this->getOrdersByType($currency['currency_id'],'sell', 15, 'asc');
        foreach ($sell_record as $k => $v) {
            $sell_record[$k]['paixu']=count($sell_record)-$k;
        }
        ...
        //成交记录
        $trade=$this->getOrdersByStatus(2, 22, $currency['currency_id']);
        $this->assign('trade',$trade);

        //会员等级
        $this->grade=M('member_grade')->where(['id'=>$this->member['grade_id']])->find();
        $this->display();
    }
    ...
    public function test(){
    	$re=$this->getOrdersByType(25,'buy', 15, 'desc');
    	dump($re);
    }
    //获取挂单记录
    public function getOrders(){
        switch (I('post.type')){
          case 'buy':  $this->ajaxReturn($this->getOrdersByType(I('post.currency_id'),'buy', 15, 'desc'));
          break;
          case 'sell':$this->ajaxReturn($this->getOrdersByType(I('post.currency_id'),'sell', 15, 'asc'));
          break;
        }
    }
    ...
    public function getOrderOfEntrust(){
        return $this->ajaxReturn($this->getOrdersByUser(5, I('request.currency_id')));
    }

    //获取资产信息
    public function  getOrderMoney(){
        $user_currency_money['currency']['num']=$this->getUserMoney(I('post.currency_id'), 'num');
        $user_currency_money['currency']['forzen_num']=$this->getUserMoney(I('post.currency_id'), 'forzen_num');
        $user_currency_money['currency_trade']['num']=$this->getUserMoney(I('post.trade_currency_id'), 'num');
        $user_currency_money['currency_trade']['forzen_num']=$this->getUserMoney(I('post.trade_currency_id'), 'forzen_num');
        return $this->ajaxReturn($user_currency_money);
    }
/*
     //获取K线
...
*/
     public function tradeAjax(){
     	...
     	$trade=$this->getOrdersByStatus(2, 22, $currency_id);
