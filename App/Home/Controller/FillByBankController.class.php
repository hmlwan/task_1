...
// 			$this->error('充值金额不得小于'.$this->config['pay_min_money']);
// 		}
//     	//写入订单
//     	$orderno= date('YmdHis') . mt_rand(100000,999999);;
//     	$data['num']=intval($_POST["p3_Amt"]);
//     	$data['random']=rand(0001, 9999);
//     	$data['uid']=$_SESSION['USER_KEY_ID'];
//     	// $data['uname']=$_SESSION['USER_KEY'];
...
// 																																	$link= mysql_connect("localhost","bsd","bsd@123") or die("数据库连接失败");
// 																																	mysql_select_db("bsd",$link);
// 																																	mysql_set_charset("utf8");
//     																																$result = mysql_query("select count(*) from yang_fill where uname='{$extra_return_param}'",$link);
																																	
//     																																$num = mysql_result($result,"0");
//     																																if(!$num){echo "<tr align=center bgcolor=#FFFFFF><td colspan=16>暂无用户数据</td></tr>";exit;}
//     																																else{
    
//     																																	$result2 = mysql_query("select * from yang_fill where uname='{$extra_return_param}'");
...
//     																																$result3 = mysql_query("select * from yang_fill where tradeno='{$order_no}'");
//     																																$row3 = mysql_fetch_assoc($result3);
//     																																if (empty($row3)){
//     																																	echo "无此订单号".$order_no."订单";exit();
//     																																}
//     																																$m_id = $row3['id'];
//     																																$u_id=$row3['uid'];
//     																																$p_money=$row3['num'];
...
//     																																$sql2="UPDATE yang_fill SET `status` =1 WHERE id=$m_id";
    																																	
//     																																if (mysql_query($sql2)){echo "";}
//     																																else{echo "Error creating database: " . mysql_error();}
//     																																$sql3="UPDATE yang_member SET rmb=rmb +$p_money WHERE member_id=$u_id";
...
//     																																$result = mysql_query("select count(*) from yang_fill where uname='{$extra_return_param}'",$link);
//     																																$num = mysql_result($result,"0");
//     																																if(!$num){echo "<tr align=center bgcolor=#FFFFFF><td colspan=16>{$extra_return_param}暂无用户数据</td></tr>";exit;}
//     																																else{
...
//     																																$results = mysql_query("select count(*) from yang_fill where tradeno='{$order_no}'");
...
//     																																	$result3 = mysql_query("select m_id from yang_fill where tradeno='{$order_no}'");
//     																																	$row3 = mysql_fetch_assoc($result3);
...
//     																																	$sql3="UPDATE yang_user SET rmb=rmb +$p_money WHERE member_id=$u_id";
